...
- (void)viewWillDisappear:(BOOL)animated {//销毁，并资源释放
...
- (void)showVideoPlayer:(UITapGestureRecognizer *)tapGesture {
    ...
    _indexPath = [NSIndexPath indexPathForRow:view.tag - 100 inSection:0];////获取到点击的那个下标
    XLVideoCell *cell = [self.exampleTableView cellForRowAtIndexPath:_indexPath];
    
    _player = [[XLVideoPlayer alloc] init];////销毁之前的，创建新的播放器
    ...
    _player.completedPlayingBlock = ^(XLVideoPlayer *player) {////播放完成
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.videoImageView.tag = indexPath.row + 100;////必须赋值，防止复用
    return cell;
}
